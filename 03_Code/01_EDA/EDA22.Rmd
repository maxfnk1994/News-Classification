---
title: "Exploratory data analysis - Classification of news headlines with impact on the probability of stock prices changes"
author: Max Franke
date: March 31, 2020
output:
    pdf_document:
...
This analysis aims to visualize the collected data to include subsequent results and relationships in the further process of the project. The following procedure is applied: 

First, a data set is examined that evaluates the mood of headlines and their descriptions concerning different stocks in a labeled form. 

Based on this, the most frequently used words from headline and description are linked to its frequency. The background to this approach is that the assumption is made that the defined words are representative of a word family that influences stocks so that the trend of these words can be examined more closely. 
...
The first graph shows the total numbers of symbols go give a visualization about the distribution of the stocks on a high level.
...
Apple, Facebook, and Amazon show the highest frequency. The sum of these three stocks is 46.9% of the total dataset. 
...
It can be seen that facebook has a high number of bad publicity (33.6% of all facebook news are labeled as a negative sentiment). 
...
First, the column "title" is extracted and saved to a txt.file. Then, this txt. file is load into R.
...
In the following table, the first 10 words of this term-document matrix are shown.
...
Furthermore, words like model, tech and, data are highlighted which is interesting and can be interpreted as an indicator for the impact of the field "data science" in the stock market.


### Step 6: Frequent terms in the term-document matrix

The following table shows the words which have the lowest frequence of 30. The table is ordered by column, which means that the first column presents the words 1-10.
...
The following table shows the top 10 words by frequency in the title.
...
This graph visualize the top 10 words in the title
...
This graph selected the cases in the dataset, which includes in the column "title" the word "update" in any form.
The big grey bar charts represents the overall frequency of the word "update", split by the three classes in the sentiment.
The colorful bar charts shows the distribution per each stock, split by the three classes in the sentiment.
...
The word "update" shows concerning the different values of sentiment kind of a normal distribution, which means that the most titles including the word "update" have a neutral sentiment. But there is a small trend that the word update leads to negative sentiment in the news title.

Split by stocks, it is shown that facebook shows a high negative frequency, which was already shown in the Graph: Number of stock symbols per sentiment.

\newpage

## Process of generating word clouds for the column description

### Step 1: Extract and then load the descriptions from dataset

First, the column "description" are extracted and saved to a txt.file. Then, this txt. file is load into R.

```{r echo=FALSE}
# Write column description to txt. file
write.table(stock_text$description, "description.txt", row.names = FALSE, col.names = FALSE)
text <- readLines("description.txt")

# Head of descriptions
kable(data.frame(Number = c(1:6), descriptions = head(text)), "latex",
      longtable = T, booktabs = T,
      caption = "First 6 descriptions") %>%
  kable_styling(latex_options = c("repeat_header")) %>%
...
For this analysis, the same packages as shown in the chapter for the titles are required.
...
# Remove the name of the stocks
docs <- tm_map(docs, removeWords, 
               c("apple", "amazon", "facebook", "google", "tesla", "microsoft", "facebooks", "apples", "inc"))
```
...
Eyecatcher of this graph is the word "said", which will be analyzed in the following chapters.
Also, "will" and "new" are highlighted. 
...
The following table shows the top 10 words by frequency in the description.
...
kable(data.frame(top_10, row.names = NULL), "latex",
      longtable = T, booktabs = T,
      caption = "Frequency of top 10 words in description") %>%
  kable_styling(latex_options = c("repeat_header"))

```

### Graph: Word frequency for the top 10 words in the description

This graph visualize the top 10 words in the descriptions
...
The word "said" is by a wide range the most frequent word in the description. So, somebody said something which has an impact on the stock

Consequently, a further analysis concerning the word "said" and sentiment would be interesting.

### Graph: The word "said" in descriptions, split by sentiment

This graph selected the cases in the dataset, which includes in the column "description" the word "said" in any form.
The big grey bar charts represents the overall frequency of the word "said", split by the three classes in the sentiment.
The colorful bar charts shows the distribution per each stock, split by the three classes in the sentiment.

```{r echo=FALSE}
# Define dataframe
p7 <- stock_text %>% drop_na()

# Add said column
p7$said <- NA

# define values for possible options of said
said <- c("said")

# check every row for values in description
for (i in 1:nrow(p7)) {
    if (str_detect(p7[i,2], "said") == TRUE) {
    p7[i,5] <- "said"
    }
}
...
# Change the class of column sentiment to change the labels in ggplot
graph7 <- p7_said
graph7$sentiment <- as.character(graph7$sentiment)

# Plot
p7_final <- ggplot(data = graph7, mapping = aes(x = sentiment)) +
        geom_bar(mapping = aes(y = ..count..), position = "dodge") +
        geom_bar(mapping = aes(y = ..count.., fill = symbol), position = "dodge", alpha = 0.6) +
        scale_x_discrete(labels = c("1" = "positive", "0" = "neutral", "-1" = "negative")) + 
        theme(legend.position = "bottom") +
        labs(fill = "Stock symbols",
             title = "Word said in descriptions divided by sentiment",
             x = "Sentiment of news article",
             y = "Frequency",
             caption = "Source: news headlines of ~5k news articles")
p7_final
...
The word “said” shows concerning the different values of sentiment kind of a normal distribution, which means that the most descriptions including the word “said” have a neutral sentiment. But there is a small trend that the word "said" leads to negative sentiment in the news description. 

Split by stocks, it is shown that facebook shows a high negative frequency, which was already shown before.

# Quick Summary

 - In this data set, which contains news, the three companies (Apple, Facebook, Amazon() were most talked about
 - The news was most often neutral in terms of sentiment (50% of all cases in the dataset)
 - Facebook shows a high frequency in the negative sentiment
 - The analysis of the titles shows that the word "update" is often used, although the distributions between the sentiments are also similarly normally distributed
 - A word family (e.g. "model", "tech" and "data") which can be connected to the field of 'Data Science' are highlighted in teh word cloud
 - The analysis of the descriptions shows that the word "said" is often used. This can mean that the description mentions that someone has said something and that this statement has an impact on the stock market.
