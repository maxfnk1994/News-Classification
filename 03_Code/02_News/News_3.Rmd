...
# Normalize with min Max the Price Difference for 3 minutes
Price_Difference_3min_norm <- zScore(Apple_Result$PD3)
# Normalize with min Max the Counts
...
```{r}
layers <- expand.grid(rep(list(c(5, 10, 20, 40, 80, 100, 200)), 3))
Result_layers <- expand.grid(rep(list(c(5, 10, 20, 40, 80, 100, 200)), 3))
...
write.csv(Result_layers, file = "Result_layers2.csv")
}
...
```{r}
epochs_results <- melt(Result_layers, id = c("Hidden1", "Hidden2"))
